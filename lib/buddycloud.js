...
Buddycloud.prototype.ERROR_SINCE_DATE_UNPARSABLE = 'Since date was unparsable'

Buddycloud.prototype.init = function(manager, ignoreEvents) {
    init.call(this, manager, ignoreEvents)
...
Buddycloud.prototype._events = {
    ...
    'xmpp.buddycloud.delete': 'removeNode',
...
Buddycloud.prototype.removeNode = function(data, callback) {
    if (!this._checkCall(data, callback)) return
...
Buddycloud.prototype.retrieve = function(data, callback) {
    ...
    if (data.parentOnly) {
        stanza.root().getChild('pubsub')
            .getChild('items')
            .attr('parent-only', 'true')
    }
    var self = this
...
Buddycloud.prototype.userFeedItems = function(data, callback) {
    if (!this._checkCall(data, callback)) return
    this.getUserFeedItems(data, callback)
}
...
Buddycloud.prototype._getSubscriptions = function(stanza, data) {
    if (!data.ephemeral) return true
    var subscriptions = stanza.root()
        .getChild('pubsub')
        .getChild('subscriptions')
    subscriptions.attr('bc:xmlns', this.NS_BUDDYCLOUD)
    subscriptions.attr('bc:ephemeral', 'true')
    return true
}
...
Buddycloud.prototype._getAffiliations = function(stanza, data) {
    if (!data.ephemeral) return true
    var affiliations = stanza.root()
        .getChild('pubsub')
        .getChild('affiliations')
    affiliations.attr('bc:xmlns', this.NS_BUDDYCLOUD)
    affiliations.attr('bc:ephemeral', 'true')
    return true
}
...
Buddycloud.prototype.discover = function(data, callback) {
    ...
    var cachedServer = this._getCachedServer(request.of, 'channel')
    if (cachedServer) {
        this.channelServer = cachedServer
        callback(null, cachedServer)
        return true
    }
            ...
            for (var i in results) {
                results[i].forEach(function(feature) {
                    if (('identity' === feature.kind) &&
                        ...
                        discovered = true
                        self.addServerCache(self.manager.getJidType('domain'), i, 'channel')
...
Buddycloud.prototype._getCachedServer = function(domain, type) {
    try {
        var cache = this._getCache()
        return cache.buddycloud.server[type][domain]
    } catch (e) {
        return false
    }
}

Buddycloud.prototype.addServerCache = function(domain, server, type) {
    var cache = this._getCache()
    if (!cache) return false
    if (!cache.buddycloud) cache.buddycloud = {}
    if (!cache.buddycloud.server) cache.buddycloud.server = {}
    if (!cache.buddycloud.server[type]) cache.buddycloud.server[type] = {}
    cache.buddycloud.server[type][domain] = server
}
...
Buddycloud.prototype.discoverMediaServer = function(data, callback) {
    ...
    var server = this._getCachedServer(domain, 'media')
    if (server && server.component) {
        this.mediaServers[domain] = server
    ...
    this.disco.getItems(request, function(error, items) {
        ...
        async.parallel(jobs, function(error, results) {
            ...
            for (var i in results) {
                ...
                if (discovered) {
                    return callback(null, self.mediaServers[domain])
                }
            }
            if (false === discovered)
...
Buddycloud.prototype._checkForMediaComponent = function(feature, domain, component) {
    if (('identity' === feature.kind) &&
        ...
        this.addServerCache(domain, this.mediaServers[domain], 'media')
        return true
    }
    return false
}
...
Buddycloud.prototype.getUserFeedItems = function(data, callback) {
    var self = this
    var since = (!data.since) ? 0 : data.since
    var parentOnly = (data.parentOnly) ? 'true' : 'false'
    since = Date.parse(since)
    var stanza = this._getStanza({ to: data.to }, 'get', 'user-items')
    if (true === isNaN(since))
        return this._clientError(
            this.ERROR_SINCE_DATE_UNPARSABLE, data, callback
        )
    stanza.attrs = {
